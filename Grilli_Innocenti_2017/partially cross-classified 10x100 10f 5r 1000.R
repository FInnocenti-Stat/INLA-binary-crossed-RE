...
sigmaB2<-sigmaUB2<-varB2<-prec.margB2<-marg.varianceB2<-sd_varB2<-sdB2<-mmB2<-NULL
...
freq.receivers<-matrix(0,sim,50)
...
for(iter in 1:sim){    
  ...
  idreceiver2.1=sample(c(1:10)[-idreceiver1[1]], 4, replace=FALSE)         # draw the other 4 receivers  (1st row (i.e. 1st feeder) of the cross-classification matrix)
  idreceiver2.2=sample(c(1:10)[-idreceiver1[2]], 4, replace=FALSE)         # draw the other 4 receivers  (2nd row (i.e. 2nd feeder) of the cross-classification matrix)
  idreceiver2.3=sample(c(1:10)[-idreceiver1[3]], 4, replace=FALSE)         # draw the other 4 receivers  (3rd row (i.e. 3rd feeder) of the cross-classification matrix)
  idreceiver2.4=sample(c(1:10)[-idreceiver1[4]], 4, replace=FALSE)         # draw the other 4 receivers  (4th row (i.e. 4th feeder) of the cross-classification matrix)
  idreceiver2.5=sample(c(1:10)[-idreceiver1[5]], 4, replace=FALSE)         # draw the other 4 receivers  (5th row (i.e. 5th feeder) of the cross-classification matrix)
  idreceiver2.6=sample(c(1:10)[-idreceiver1[6]], 4, replace=FALSE)         # draw the other 4 receivers  (6th row (i.e. 6th feeder) of the cross-classification matrix)
  idreceiver2.7=sample(c(1:10)[-idreceiver1[7]], 4, replace=FALSE)         # draw the other 4 receivers  (7th row (i.e. 7th feeder) of the cross-classification matrix)
  idreceiver2.8=sample(c(1:10)[-idreceiver1[8]], 4, replace=FALSE)         # draw the other 4 receivers  (8th row (i.e. 8th feeder) of the cross-classification matrix)
  idreceiver2.9=sample(c(1:10)[-idreceiver1[9]], 4, replace=FALSE)         # draw the other 4 receivers  (9th row (i.e. 9th feeder) of the cross-classification matrix)
  idreceiver2.10=sample(c(1:10)[-idreceiver1[10]], 4, replace=FALSE)       # draw the other 4 receivers  (10th row (i.e. 10th feeder) of the cross-classification matrix)
  
  # to ensure that all the 10 receivers are drawn at least one time (no empty column in the cross-classification matrix)
  receivers[1:n1]<-c(rep(idreceiver1[1],each=20),rep(idreceiver2.1[1],each=20),rep(idreceiver2.1[2],each=20),rep(idreceiver2.1[3],each=20),rep(idreceiver2.1[4],each=20))
  receivers[(n1+1):(n1*2)]<-c(rep(idreceiver1[2],each=20),rep(idreceiver2.2[1],each=20),rep(idreceiver2.2[2],each=20),rep(idreceiver2.2[3],each=20),rep(idreceiver2.2[4],each=20))
  receivers[((n1*2)+1):(n1*3)]<-c(rep(idreceiver1[3],each=20),rep(idreceiver2.3[1],each=20),rep(idreceiver2.3[2],each=20),rep(idreceiver2.3[3],each=20),rep(idreceiver2.3[4],each=20))
  receivers[((n1*3)+1):(n1*4)]<-c(rep(idreceiver1[4],each=20),rep(idreceiver2.4[1],each=20),rep(idreceiver2.4[2],each=20),rep(idreceiver2.4[3],each=20),rep(idreceiver2.4[4],each=20))
  receivers[((n1*4)+1):(n1*5)]<-c(rep(idreceiver1[5],each=20),rep(idreceiver2.5[1],each=20),rep(idreceiver2.5[2],each=20),rep(idreceiver2.5[3],each=20),rep(idreceiver2.5[4],each=20))
  receivers[((n1*5)+1):(n1*6)]<-c(rep(idreceiver1[6],each=20),rep(idreceiver2.6[1],each=20),rep(idreceiver2.6[2],each=20),rep(idreceiver2.6[3],each=20),rep(idreceiver2.6[4],each=20))
  receivers[((n1*6)+1):(n1*7)]<-c(rep(idreceiver1[7],each=20),rep(idreceiver2.7[1],each=20),rep(idreceiver2.7[2],each=20),rep(idreceiver2.7[3],each=20),rep(idreceiver2.7[4],each=20))
  receivers[((n1*7)+1):(n1*8)]<-c(rep(idreceiver1[8],each=20),rep(idreceiver2.8[1],each=20),rep(idreceiver2.8[2],each=20),rep(idreceiver2.8[3],each=20),rep(idreceiver2.8[4],each=20))
  receivers[((n1*8)+1):(n1*9)]<-c(rep(idreceiver1[9],each=20),rep(idreceiver2.9[1],each=20),rep(idreceiver2.9[2],each=20),rep(idreceiver2.9[3],each=20),rep(idreceiver2.9[4],each=20))
  receivers[((n1*9)+1):(n1*10)]<-c(rep(idreceiver1[10],each=20),rep(idreceiver2.10[1],each=20),rep(idreceiver2.10[2],each=20),rep(idreceiver2.10[3],each=20),rep(idreceiver2.10[4],each=20))
  
  # store the drawn receivers for each replicate
  freq.receivers[iter,]<-recei<-c( idreceiver1[1],idreceiver2.1,
                                   idreceiver1[2],idreceiver2.2,
                                   idreceiver1[3],idreceiver2.3,
                                   idreceiver1[4],idreceiver2.4,
                                   idreceiver1[5],idreceiver2.5,
                                   idreceiver1[6],idreceiver2.6,
                                   idreceiver1[7],idreceiver2.7,
                                   idreceiver1[8],idreceiver2.8,
                                   idreceiver1[9],idreceiver2.9,
                                   idreceiver1[10],idreceiver2.10)
  ...
  for(s in 1:50){           # assign each receiver to the corresponding random effect
    for(t in 1:50){
  ...
  rand<-rep(U,each=n1)     #   Feeders random effects
                           #   Receivers random effects
  part<-c(rep(V[1],each=20),rep(V[2],each=20),rep(V[3],each=20),rep(V[4],each=20),
          rep(V[5],each=20),rep(V[6],each=20),rep(V[7],each=20),rep(V[8],each=20),
          rep(V[9],each=20),rep(V[10],each=20),rep(V[11],each=20),rep(V[12],each=20),
          rep(V[13],each=20),rep(V[14],each=20),rep(V[15],each=20),rep(V[16],each=20),
          rep(V[17],each=20),rep(V[18],each=20),rep(V[19],each=20),rep(V[20],each=20),
          rep(V[21],each=20),rep(V[22],each=20),rep(V[23],each=20),rep(V[24],each=20),
          rep(V[25],each=20),rep(V[26],each=20),rep(V[27],each=20),rep(V[28],each=20),
          rep(V[29],each=20),rep(V[30],each=20),rep(V[31],each=20),rep(V[32],each=20),
          rep(V[33],each=20),rep(V[34],each=20),rep(V[35],each=20),rep(V[36],each=20),
          rep(V[37],each=20),rep(V[38],each=20),rep(V[39],each=20),rep(V[40],each=20),
          rep(V[41],each=20),rep(V[42],each=20),rep(V[43],each=20),rep(V[44],each=20),
          rep(V[45],each=20),rep(V[46],each=20),rep(V[47],each=20),rep(V[48],each=20),
          rep(V[49],each=20),rep(V[50],each=20))
...
freq.emptycol 
sum(freq.emptycol)  
hist(freq.emptycol,ylim=c(0,500),xlim=c(0,4),main="Empty columns",xlab="empty columns")  
